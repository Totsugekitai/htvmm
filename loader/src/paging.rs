...
unsafe fn construct_table_inner(
    ...
    for i in 0..512 {
        ...
        vmm_page_table[i].set_addr(
            x86_64::PhysAddr::new(sub_vmm_page_table as *const PageTable as u64),
...
unsafe fn modify_table(
    ...
    for i in 0..(ALLOCATE_BYTES_FOR_VMM / 0x200000) {
        ...
        pde.set_addr(
            x86_64::PhysAddr::new(vmm_entry_phys + 0x20_0000 * i as u64),
    ...
    vmm_pdpte4.set_addr(
        x86_64::PhysAddr::new(pd_table as *mut PageTable as u64),
        vmm_pdpte4_flags,
    );
}
